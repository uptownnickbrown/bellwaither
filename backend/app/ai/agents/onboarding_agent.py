...
Conducts a brief conversation to understand the school's context,
then proposes a customized framework (full SQF + targeted modifications).
"""
...
ONBOARDING_SYSTEM_PROMPT = """You are the Meridian Framework Advisor. You help schools customize their assessment framework through a brief, focused conversation.

THE CORE IDEA: Every school starts with the FULL Bellwether School Quality Framework (SQF) — all 9 dimensions, all 43 components. Your job is to learn enough about the school to make TARGETED MODIFICATIONS:
- Tailor descriptions and criteria to this school's context
- Add 1-3 custom dimensions or components that reflect their unique identity (e.g., Quaker Identity, DLI Program, STEM Focus)
- Add custom criteria to existing components where the school has specific priorities
- Optionally remove a small number of components that are truly irrelevant (rare — most components matter for most schools)

You are NOT building from scratch. You are customizing a comprehensive, proven framework.

THE FULL SQF (this is the BASELINE — include ALL of it unless there's a strong reason to cut):
...
CONVERSATION RULES:
1. This is a 2-3 turn conversation, NOT an interrogation. Two questions max before you propose.
...
4. Turn 3 (or Turn 2 if they said enough): PROPOSE. Do not ask another question. Build the framework.
5. If the user says "I don't know", "just give me the framework", or seems impatient: PROPOSE IMMEDIATELY with your best guess based on the school profile.
...
RESPONSE FORMAT (proposal) — THIS IS THE IMPORTANT ONE:
{
  "status": "proposal",
  "framework": {
    "dimensions": [ ... all dimensions with all components and criteria ... ]
  },
  "rationale": "What I customized and why: [specific changes]. The full SQF baseline is preserved."
}

PROPOSAL RULES — READ CAREFULLY:
1. ALWAYS include ALL 9 standard SQF dimensions. Do not cut dimensions unless the user explicitly asked to remove one.
2. ALWAYS include ALL standard components within each dimension. The baseline is the full 43-component SQF.
3. ADD 1-3 custom dimensions or custom components based on what you learned (is_custom: true). These are the customizations.
...
9. A proposal with fewer than 8 dimensions or fewer than 30 components is TOO THIN. The whole point of the SQF is comprehensive assessment.
...
async def conduct_interview(
    ...
    """Process one turn of the onboarding interview.

    Args:
        school_profile: Basic school info (name, type, grades, etc.)
        conversation_history: List of {role, content} messages so far

    Returns:
        dict with either:
        - {"status": "interviewing", "message": "...", "turn": N}
        - {"status": "proposal", "framework": {...}, "rationale": "..."}
    """
...
"""
    ...
    # Count user turns to enforce the limit
    ...
    # If we've had 3+ user turns, force a proposal by adding a nudge
    if user_turns >= 3:
        messages.append({
            "role": "system",
            "content": "You have had enough conversation. You MUST respond with status=proposal now. Do not ask another question. Build the full framework.",
        })
    ...
    # Assign colors to dimensions in proposal
    if result.get("status") == "proposal" and result.get("framework"):
        dims = result["framework"].get("dimensions", [])
        for i, dim in enumerate(dims):
            if not dim.get("color"):
                dim["color"] = DIMENSION_COLORS[i % len(DIMENSION_COLORS)]

    logger.info("Onboarding interview result: status=%s model=%s", result.get("status"), model)
    return result
